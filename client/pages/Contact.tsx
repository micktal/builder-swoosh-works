...
import { AtSign, Building2, CheckCircle2, MapPin, Phone, User } from "lucide-react";
import { useState } from "react";

export default function Contact() {
  const heroImg = "https://images.unsplash.com/photo-1551836022-d5d88e9218df?q=80&w=1600&auto=format&fit=crop";
    ...
    <main>
      ...
      <section className="relative overflow-hidden bg-[#0E1E2B] text-white">
        ...
        <div className="container-padded py-16 md:py-24 grid md:grid-cols-2 gap-8 items-center">
          ...
          <img src={heroImg} alt="Équipe FPSG" className="hidden md:block rounded-[10px] shadow-[0_12px_32px_rgba(0,0,0,0.1)]" />
        </div>
      </section>
      ...
      <section className="bg-[#F9FAFB]">
        <div className="container-padded py-12 md:py-20">
          ...
          <Reveal delay={80}>
            <p className="mt-2 text-muted-foreground">Remplissez le formulaire ci-dessous, notre équipe vous recontactera sous 24 heures.</p>
          </Reveal>
          ...
          <div className="mt-8 mx-auto max-w-3xl">
            {sent ? (
              <div className="card p-6 text-center">
                ...
                <p className="mt-2 text-muted-foreground">Notre équipe FPSG vous contactera très prochainement.</p>
                <button className="mt-4 btn-outline-green" onClick={() => setSent(false)}>Envoyer une nouvelle demande</button>
              </div>
            ) : (
              <form onSubmit={onSubmit} className="card p-6 bg-white">
                {error && (
                  <div className="mb-4 rounded-[8px] border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</div>
                )}
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2 rounded-[10px] border bg-white px-3 py-2">
                      ...
                      <input id="name" name="name" required className="w-full bg-transparent focus:outline-none" placeholder="Ex: Marie Dupont" />
                    </div>
                  </div>
                  <div>
                    <label className="text-sm font-medium" htmlFor="company">Entreprise / Organisation</label>
                    <div className="mt-1 flex items-center gap-2 rounded-[10px] border bg-white px-3 py-2">
                      <Building2 className="h-4 w-4 text-muted-foreground" />
                      <input id="company" name="company" className="w-full bg-transparent focus:outline-none" placeholder="Ex: FIDUCIAL" />
                    </div>
                  </div>
                  <div>
                    <label className="text-sm font-medium" htmlFor="email">Adresse e-mail professionnelle</label>
                    <div className="mt-1 flex items-center gap-2 rounded-[10px] border bg-white px-3 py-2">
                      <AtSign className="h-4 w-4 text-muted-foreground" />
                      <input id="email" name="email" type="email" required className="w-full bg-transparent focus:outline-none" placeholder="prenom.nom@entreprise.fr" />
                    </div>
                  </div>
                  <div>
                    <label className="text-sm font-medium" htmlFor="phone">Téléphone (optionnel)</label>
                    <div className="mt-1 flex items-center gap-2 rounded-[10px] border bg-white px-3 py-2">
                      <Phone className="h-4 w-4 text-muted-foreground" />
                      <input id="phone" name="phone" className="w-full bg-transparent focus:outline-none" placeholder="Ex: +33 6 12 34 56 78" />
                    </div>
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium" htmlFor="subject">Sujet</label>
                    <select id="subject" name="subject" required className="mt-1 w-full rounded-[10px] border bg-white px-3 py-2">
                  ...
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium" htmlFor="message">Message libre (vos besoins)</label>
                    <textarea id="message" name="message" rows={5} className="mt-1 w-full rounded-[10px] border bg-white px-3 py-2" placeholder="Décrivez votre contexte, vos objectifs et publics cibles..." />
                  </div>
                </div>
        ...
        <div className="container-padded py-12 md:py-20">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            <div className="card p-6">
              ...
              <p className="mt-1 text-sm text-muted-foreground">FIDUCIAL FPSG – Siège<br/>41 rue du Capitaine Guynemer, 69002 Lyon</p>
            </div>
            <div className="card p-6">
              <Phone className="h-6 w-6 text-primary" />
              <h3 className="mt-2 font-heading font-semibold">Téléphone</h3>
              <p className="mt-1 text-sm text-muted-foreground">+33 (0)4 72 57 20 00</p>
            </div>
            <div className="card p-6">
              <AtSign className="h-6 w-6 text-primary" />
              <h3 className="mt-2 font-heading font-semibold">E-mail</h3>
              <p className="mt-1 text-sm text-muted-foreground">contact@fpsg.fr</p>
            </div>
          </div>
          <div className="mt-8">
            <a className="btn-cta" href="https://maps.google.com?q=41+rue+du+Capitaine+Guynemer,+69002+Lyon" target="_blank" rel="noreferrer">Ouvrir Google Maps</a>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white">
        <div className="container-padded py-12 md:py-20">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {[
              { t: "Accessibilité", d: "Formations inclusives, accessibles à tous." },
              { t: "Réactivité", d: "Réponse sous 24h à toute demande de démo." },
              { t: "Expertise terrain", d: "Contenus développés par nos formateurs FPSG." },
            ].map((c) => (
              <div key={c.t} className="rounded-[10px] bg-[#E5F4ED] p-6 text-[#006B46] shadow-[0_8px_24px_rgba(0,0,0,0.1)]">
